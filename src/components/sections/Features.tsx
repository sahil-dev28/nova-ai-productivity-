      ...
      <Reveal stagger>
        <ul className="mt-gap-cards grid grid-cols-[repeat(auto-fit,minmax(min(300px,100%),1fr))] gap-4">
          {features.map(({ title, description, icon: Icon }, index) => (
            ...
            >
              <Card className="h-full gap-4 border border-line ring-0 [--card-spacing:24px] transition-[translate,border-color,box-shadow] duration-500 ease-card hover:-translate-y-1.5 hover:border-accent-40 hover:shadow-lift">
